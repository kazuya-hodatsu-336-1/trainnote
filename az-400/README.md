...
* **<span style="color: red; ">ラボ用環境の注意</span>**  
  1. **このページの下部に、Labごとの補足事項やうまくいかない場合のフォロー情報が書いてあります。Labを進める前に一読してから、Lab本体のコンテンツを進めてください。（Lab本体はMicrosoftから提供されている資料で、その資料にわかりづらい点や混乱する点がある個所を本誌で補足しています）**
  1. 【重要】Lab内のIME入力モードと、Lab外のIME入力モード。   
  1.1  Lab外のIME入力モードは、「半角英数字/直接入力」にしてください。Lab内のIME入力モードは、タスクバーに表示されている、IMEモードのアイコン（「A」や「あ」）をクリックして切り替えてください。（キーボードのショートカットで切り替えたりすると、Lab環境と利用端末間でキーボードが衝突して、不正な挙動になります）
  2. 【重要】ラボの右側に、デモ用のガイドが出てきます。ですが、講義の進行順序と、ラボのガイドの内容・ラボの番号が一致していない可能性があります。原則的に、GithubのWebページとしてお伝えしている「ラボコンテンツ」を正としてください。ただし、一部ラボ内のコンテンツのほうが正しい場合があります。うまく進められない場合は適宜ご質問ください。
  3. 前述の通り、ラボ内のガイドと、ラボ外のガイドで、ラボの番号が異なります。タイトルを見て判断してください。（Microsoftメンテナンスチームが常に更新を続けているため、更新が追い付かない場合があります。）
  4. キーボードの配列がご利用いただいているキーボードと異なっている可能性があります。仮想キーボードをONにすることで、どこがどうなっているのかわかりますので、参照してください。  
  ![Lab仮想キーボード01](./img/Lab準備_仮想キーボード01.jpg)  
  ![Lab仮想キーボード02](./img/Lab準備_仮想キーボード02.jpg)  
  5. Lab外でコピーしたテキストは、Lab内に直接ペーストできません。以下の手順で実施してください。
    5.1. ペーストしたいフィールドを選択状態にまずはする。  
    5.2. Labウィンドウ左上の稲妻マーク（コマンド）をクリック  
    ![Labテキスト貼り付け01](./img/Lab準備_テキスト貼り付け01.jpg)  
    5.3. コマンドリストから、[テキストを入力] > [クリップボードのテキストを入力します。]を選択。
    ![Labテキスト貼り付け01](./img/Lab準備_テキスト貼り付け02.jpg)  
    5.4. Lab側のアクティブなコントロールに入力したいテキストを、表示されたダイアログに入力して「はい」をクリック。
    ![Labテキスト貼り付け01](./img/Lab準備_テキスト貼り付け03.jpg)  
    5.5. Lab側に、テキストが反映されます。
...
* **ラボ 13:機能テストの設定と実行**  
  * 演習0
    * タスク2  
      * 事前にResource groupsを作成してください。リソースグループのリージョンをEast USなどにしておいてください。（事前に作っておかないと、新規でカスタムテンプレートから作るとエラーになります）
      * 3.「カスタム デプロイ」ブレードで、「パラメーターの編集」を選択します。
        * 正しくは「テンプレートの編集」です。
  * 演習1
    * タスク1
      * RDPにログインするとき、利用しているキーボードと「@」などの位置が違う可能性があるので、ログインできない時は、仮想キーボードを表示して場所を確認してください。
      * 4.az40011bvm へのリモート デスクトップ セッション内で Chrome Web ブラウザー ウィンドウを開き、 https://dev.azure.com に移動して Azure DevOps 組織にサインインします。
        * ログインする際に、Switch Directoryリンクをクリックして、既定のディレクトリにSwitchしてください。
      * 12.管理者:コマンド プロンプト ウィンドウで、以下を実行してエージェント バイナリのインストールを開始します。
        * 「￥」の半角がうまく打てない時は、「/」でもOKです。
    * タスク3
      * 6.Selenium テスト実行 フェーズが始まったら、Web ブラウザー テストを監視します。
        * RDPがかたまり、切断されてしまう場合があります。ダウンロードフォルダにRDP用のファイルがダウンロードされているはずですので、再度接続を試みてください。
          * Lab側のネットワークが固まっている場合がありえます。その場合はLabを再起動する必要があります。（タスクバーの地球儀のアイコンを見て、ネットワークが切断されていれば再起動が必要。再起動してもダメな場合は、機械の電源を切る で一度完全に切り、開始ボタンをクリックする。それでもダメな場合は、お手数ですがご自身のローカル端末で続行してください。）
        * テスト結果はエラーとなる場合がありますが、今回はSeleniumを用いた自動試験の実行のため、完了として次へ進んでください。

---
* **ラボ 14:Ansible と Azure**  
  * 演習1
    * タスク2  
      * 3.以下を実行して、Ansible と必要な Azure モジュールをインストールします (コマンドを 1 行ずつ個別に実行し、確認を求められたら、y と入力して Enter キーを押します)。
        * 「y」ではなく、「Y」と選択肢が出ている場合があるので注意。「Y」「y」は別物です。（Abortしてしまった場合は、コマンドをやり直してください）
    * タスク6
      * 【重要】3.以下を実行して、Code テキスト エディターで new_vm_web.yml ファイルを開きます
        * SSH越しのCode表示は失敗する場合が大半です。（Azure Cloud Shellが対応できていない場合が多い）以下の手順で、Codeを使う個所については、ラボマシンにインストールされている、VSCodeを使って、以降の処理は読み替えて実施してください。
          * VSCodeによるSSH設定
            ...
            5. SSH TARGETSと書かれているあたりにフォーカスを当てると表示される「＋」をクリック。画面上部に、Enter SSH Connection Commandという欄が出るので、「ssh azureuser@<IPアドレス>」と入力してエンター。
              * <IPアドレス>は、ラボのタスク１の６でPIPという変数に取得されています。echo $PIPでIPを確認できますので、確認して埋めてください。
            6. Edit Custom SSH configuration fileと出るので、C:\Users\Student\.ssh\configを選択
            7. 右下に、「Host added!」とでる。
            8. ついで、上記5で「＋」を押した場所の右隣に表示される、Configureアイコンをクリックし、「Settings」をクリック
            9. Remote.SSH: Config Fileの項に、「C:\Users\Student\.ssh\config」と入力
...
---
* **ラボ 17:Azure Artifacts によるパッケージ管理**  
  * 演習１
    * タスク２
      * 「クラスライブラリ(.NET Framework)」とありますが、「クラスライブラリ(.NETStandar)」ではありませんのでご注意ください。
    * タスク３
      * 4.パッケージを選択し、PartsUnlimited.Shared ペインで、 [インストール] をクリックしてプロジェクトに追加します。
        * パッケージ一覧に「PartsUnlimited.Shared」が含まれていますので探して、インストールしてください。
      * 16. かなり待ってからでなければ成功しない時があります。うまくいかない時はこのVisualStudioを開いたままにして次の講義に移り、あとで再実行してみてください。
...
* **ラボ 19**  
  * 演習１
    * タスク３
      * 14 コマンド出力の「External-IP」列で IP アドレスの値をメモし、新しい Web ブラウザー タブを開きます。その IP アドレスを参照し、MyHealthClinic アプリケーションが実行中であることを確認します。
        * kubectl get pods -Aで、mhc-frontが、ImagePullBackOffとなる現象が検出されています。現在原因調査中です。今回はリリースの流れの一連操作の確認ですので、External-IPを指定してブラウザで表示しようとしても、表示できない場合は、スキップしてラボを終了してください。
---
* **ラボ 20**  
  * 開始する前に
    * Microsoft Teams サインアップ ページ から無料のトライアル サブスクリプションを作成します。
      * 以下の手順でMicrosoft 365のサブスクリプションを作成し、これまでのMicrosoftアカウントとは異なるアカウントで、Azure Devops・Teamsにログインしていく必要があります。なお、ラボ資料自体のAzure Devopsや、Azure DevOps Demo Generatorは、この流れで作る「.onmicrosoft.com」がつくほうのアカウントを使ってください。
        * Azure Devops
          1. 以下を開きます  
            https://www.microsoft.com/ja-jp/microsoft-365/enterprise/office-365-e5?activetab=pivot:overviewtab
          2. 無料試用版をクリック
          ![200001](./img/200001.jpg)
          3. ラボ用に作成したMicrosoftの無料アカウントのメールアドレスを入力  
          ![200002](./img/200002.jpg)
          4. メールアドレスが正しければ、以下のように表示されるので「アカウントのセットアップ」をクリック
          ![200003](./img/200003.jpg)
          5. 必要情報を入力して、「次へ」をクリック
          ![200004](./img/200004.jpg)
          6. 実在するご自身の電話番号を入力して「確認コードを送信」をクリック
          ![200005](./img/200005.jpg)
          7. 電話に届いたコードを入力して「確認」をクリック
          ![200006](./img/200006.jpg)
          8. サインインする方法で、ユーザ名・ドメイン名に、AzurePassでプロモコードの引換を行った、Micorosftの無料アカウントのメールアドレスの、ドメイン部分より前のアカウント名の部分（khaz4000509@outlook.comなら「khaz4000509」）を、ユーザ名とドメイン名に入力。パスワードは任意。「次へ」をクリック
          ![200007](./img/200007.jpg)
          9. 次のような画面が表示されます。
          ![200008](./img/200008.jpg)
          10. [Azure Devops](http://dev.azure.com/)に、上記「8」で作成したアカウント（この例では、khaz4000509@khaz4000509.onmicrosoft.com）を使ってログインします。（無料で作成したMicrosoftアカウントのkhaz4000509@outlook.comだと個人アカウントになってしまい、組織アカウントになりません）
          ![200010](./img/200012.jpg) 
          11. 必要情報を入力する画面が出たら、連絡先が新たに作った組織のドメインになっていることを確認して、「続行」をクリック。パスワードも入力して続行。
          ![200011](./img/200011.jpg)
          12. Azure DevOpsを開始するためのログイン画面が表示されるので、デフォルトを受け入れてすすめる。
          ![200013](./img/200013.jpg)
          13. Azure DevOpsの画面が開いたら、新しい組織を作ったことになっているので、画面左下の「Organization settings」をクリック。
          14. 左側のナビゲーションペインで、[Security] > [Policies]をクリック。
          15. 「Third-party application access via OAuth」と、「Allow public projects」を「ON」に設定。  
          Azure Devops側はこれで完了（このデモだけ、このアカウントを使います。次回からはこのアカウントを使いません）
        * Teams  （ラボ端末の中で実施してください。ローカルで実施すると、現在参加しているこのmeetingから退出してしまう可能性があります）
          1. Teams(work or school)を起動し、Azure Devopsのほうで作成した、.onmicrosoft.comがつくほうのアカウントでログイン。
          ![200014](./img/200014.jpg)
          2. パスワードを入力すると以下の画面が表示されるので、デフォルトのままで「OK」をクリック
          ![200015](./img/200015.jpg)
          3. 「準備が完了しました！」と出たら「完了」をクリック。組織モードのTeamsが起動するので、そこからスタート。  
        * Azure DevOps Demo Generator
          1. 手順通りに作成しようとするとエラーになります。
          2. https://marketplace.visualstudio.com/search?term=ARM&target=AzureDevOps&category=All%20categories&sortBy=Relevance を開いて、「ARM」で検索して、「ARM Output」をインストールしてください。
          ![200016](./img/200016.jpg)
          ![200017](./img/200017.jpg)
          3. この画像ではARM Outputがインストール済みのため、赤枠内にボタンが出ていませんが、ボタンが出ていればクリックして、組織にインストールしてください。
          ![200018](./img/200018.jpg)
          4. そのあと、Azure DevOps Demo Generatorで、デモプロジェクトを作成してください。
          ※Generatorでエラーが出たら、プロジェクト名は「Tailwind Traders」ではなく「Tailwind Traders2」など、末尾に番号を付与してください
  * 各種演習
    * EdgeのURLアドレスバーから、アドレスをコピーする作業があるが、単純にコピーしてきて、Teamsの投稿エリアに張り付けると、アドレスではなく短縮がはられることがある。一度Notepadなどに張り付けてから、URL形式になっていることを確認したうえで、Teamsの投稿エリアに貼り付けることをお勧めします。  
    
    [確認中]
    * ラボの最後の、以下の2つについては実施できません（クレジットカード登録が必要になります）実際はパイプラインを実行すると、その結果がTeamsに投稿されると認識してください。  
      * 7.Azure DevOps ポータルの Tailwind Traders プロジェクトが表示されている Web ブラウザーに切り替え、Azure DevOps ポータルの一番左側にある垂直メニュー バーで [パイプライン] をクリックし、[パイプライン] ペインで、 [Website-CI] エントリをクリックします。その後、[Website-CI] ペインの使用中に、 [パイプラインの実行] > [実行] をクリックします。
      * 8.Teams チャネルでは、想定される動作として、パイプラインの失敗した実行に関する通知が投稿されます (パイプラインがセットアップされていない)。
...
---
* **ラボ 24:SonarCloud および Azure DevOps による技術的負債の管理**  
  * 演習１
    * タスク２
